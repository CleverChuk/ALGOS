...
class TradingPlan(tk.Tk):
	...
	def __init__(self, *arg, **kwargs):
		...
		self.after(3000, lambda:self.next_page(App_Page));
...
class Home_Page(tk.Frame):
        ...
        def showImg(self):                
                self.image = Image.open('fx.png').resize((600,650));
...
##		self.update();
		...
		cont.grid(row = 2, column = 0, columnspan = 4, rowspan = 4, sticky = 'NSWE');
		...
		global page_label;	global date_label;	global pair_label; global strategy_label; global notes_label;
		global date;
		global text;
		global overwrite_var;
		...
		# self.button_One = tk.Button(self, text = 'Back', command = lambda: controller.next_page(Home_Page));
		# self.button_One.grid(row = 6, column = 1, sticky = 's', padx = 2, pady = 2);

		self.button_Two = ttk.Button(cont, text = 'Prev', command = self.prev);
		self.button_Two.grid(row = 7, column = 1, sticky = 'sw', padx= 2, pady = 2);

		self.button_Three = ttk.Button(cont, text = 'Next', command = self.next);
		self.button_Three.grid(row = 7, column = 1, sticky = 'se', pady = 2, padx = 2);
